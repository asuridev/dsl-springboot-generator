...
public class ProductV1Controller {
    ...
    public void addProductImage(@PathVariable String productId, @RequestPart(value = "file") MultipartFile file) {
        log.info("addProductImage — productId: {}", productId);
        if (file == null || file.isEmpty()) {
            throw new BadRequestException("file: file part is required");
        }
        if (file != null && file.getSize() > 10485760L) {
            throw new BadRequestException("file: file exceeds max size 10MB");
        }
        if (
            file != null &&
            !file.isEmpty() &&
            !java.util.Set.of("image/png", "image/jpeg").contains(file.getContentType())
        ) {
            throw new UnsupportedMediaTypeStatusException(
                "file: unsupported content type — allowed: image/png, image/jpeg"
            );
        }
        useCaseMediator.dispatch(new AddProductImageCommand(productId, file));
    }
