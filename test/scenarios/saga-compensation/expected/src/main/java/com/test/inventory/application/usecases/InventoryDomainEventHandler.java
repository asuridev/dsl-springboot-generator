...
public class InventoryDomainEventHandler {
     ...
     */
    @TransactionalEventListener(phase = TransactionPhase.AFTER_COMMIT)
    @SagaStep(saga = "FulfillmentSaga", order = 1, event = "StockReservationFailed", role = SagaStep.Role.FAILURE)
    ...
    /**
     * Handles {@link StockReleasedEvent} after the wrapping transaction commits.
     * derived_from: domainEvents.published.StockReleased
     */
    @TransactionalEventListener(phase = TransactionPhase.AFTER_COMMIT)
    @SagaStep(saga = "FulfillmentSaga", order = 1, event = "StockReleased", role = SagaStep.Role.COMPENSATION)
