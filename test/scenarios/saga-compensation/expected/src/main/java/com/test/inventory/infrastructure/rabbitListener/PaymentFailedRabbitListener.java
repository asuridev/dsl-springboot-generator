...
/**
 * RabbitMQ listener for queue ${queues.inventory-payment-failed}.
 * Consumes events produced by: payments.
 * Dispatches to use case: ReleaseStock.
 * derived_from: domainEvents.consumed.PaymentFailed
 */
@Component("inventory.PaymentFailedRabbitListener")
public class PaymentFailedRabbitListener {

    private static final Logger log = LoggerFactory.getLogger(PaymentFailedRabbitListener.class);

    private final UseCaseMediator useCaseMediator;
    private final ObjectMapper objectMapper;

    public PaymentFailedRabbitListener(UseCaseMediator useCaseMediator, ObjectMapper objectMapper) {
        this.useCaseMediator = useCaseMediator;
        this.objectMapper = objectMapper;
    }

    @RabbitListener(queues = "${queues.inventory-payment-failed}")
    ...
    public void handle(Message message, Channel channel) throws IOException {
        ...
        try {
            useCaseMediator.dispatch(new ReleaseStockCommand(orderId));
