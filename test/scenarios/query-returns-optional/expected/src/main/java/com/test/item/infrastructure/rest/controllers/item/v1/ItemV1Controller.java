...
import com.test.item.application.dtos.Optional[Item];
...
public class ItemV1Controller {
    ...
    public ResponseEntity<ItemResponseDto> findItemByName(@RequestParam(required = true) String name) {
        ...
        return useCaseMediator.dispatch(new FindItemByNameQuery(name)).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    public Optional[Item] findOrCreateItem(@Valid @RequestBody FindOrCreateItemCommand command) {
