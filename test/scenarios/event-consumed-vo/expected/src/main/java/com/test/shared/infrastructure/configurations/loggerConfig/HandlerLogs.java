package com.test.shared.infrastructure.configurations.loggerConfig;

import com.test.shared.domain.annotations.*;
...
public class HandlerLogs {
    ...
    public Object executionLoggableMethod(ProceedingJoinPoint joinPoint, Loggable loggable) throws Throwable {
        ...
        if (loggable.before()) {
            log(logger, loggable.level(), "▶ Entering {} with args: [{}]", methodName, formatArgs(joinPoint, excluded));
        }
        ...
        try {
            ...
            if (loggable.timer()) {
                long executionTime = System.currentTimeMillis() - start;
                log(logger, loggable.level(), "⏱ Method {} executed in {} ms", methodName, executionTime);
            }
    ...
    private String formatArgs(JoinPoint joinPoint, Set<String> excludedArgs) {
        ...
        for (int i = 0; i < paramNames.length; i++) {
            joiner.add(paramNames[i] + "=" + (excludedArgs.contains(paramNames[i]) ? "[PROTECTED]" : args[i]));
        }
